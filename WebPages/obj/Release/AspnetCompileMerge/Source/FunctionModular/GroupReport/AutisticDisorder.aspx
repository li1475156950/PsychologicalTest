<%@ Page Title="" Language="C#" MasterPageFile="~/Main.Master" AutoEventWireup="true" CodeBehind="AutisticDisorder.aspx.cs" Inherits="WebPages.FunctionModular.GroupReport.AutisticDisorder" %>
...
<asp:Content ID="Content1" ContentPlaceHolderID="ContentPages" runat="server">
                        ...
                        <div class="table-responsive" style="text-align: left;">
                            ...
                            <table class="table table-striped table-bordered table-hover" style="text-align: center; min-width: 400px;" id="peopleSpread">
                                ...
                                <tr>
                                    <td>正常</td>
                                    <td id="normalRate"></td>
                                    <td rowspan="3" id="warnRate" style="vertical-align: middle"></td>
                                </tr>
                                <tr>
                                    <td>轻-中度孤独症</td>
                                    <td id="midRate"></td>
    ...
    <script>
        ...
        function DrawGroupAverageStateTable() {
            var grID = GetQueryString('grID');
            $.ajax({
                url: "Ajax/AutisticDisorder.ashx?OperationType=InitiAverageStateTable",
                type: "GET",
                data: { grID: grID },
                dataType: "text",
                async: false,
        ...
        function GetAverageChartData() {
            ...
            $.ajax({
                url: "Ajax/AutisticDisorder.ashx?OperationType=GetAverageChartData",
                type: "GET",
                dataType: "json",
                async: false,
                success: function (data) {
        ...
        function GetRateChartData() {
            ...
            var grID = GetQueryString('grID');
            $.ajax({
                url: "Ajax/AutisticDisorder.ashx?OperationType=GetCategory",
                type: "GET",
                async: false,
                dataType: "json",
                success: function (data) {
            ...
            $.ajax({
                url: "Ajax/AutisticDisorder.ashx?OperationType=GetScoreList",
                type: "GET",
                dataType: "json",
                data: { grID: grID },
                async: false,
                success: function (data) {
                    ...
                    var maxScoreOptionCount = data.MaxScoreOptionCount;
                    var normalCount = 0;
                    var midCount = 0;
                    var heightCount = 0;
                    var normalRate = 0;
                    ...
                    for (var i = 0; i < scoreList.length; i++) {
                        ...
                        if (scoreList[i] >= 30 && scoreList[i] < 36) {
                            midCount++;
                        }
                        if (scoreList[i] >= 36 && maxScoreOptionCount[i] < 5) {
                            midCount++;
                        }
                        if (scoreList[i] >= 36 && maxScoreOptionCount[i] >= 5) {
                            heightCount++;
                    ...
                    normalRate = normalCount / scoreList.length;
                    normalRate = normalRate.toFixed(4);
                    midRate = midCount / scoreList.length;
                    midRate = midRate.toFixed(4);
                    heightRate = heightCount / scoreList.length;
                    heightRate = heightRate.toFixed(4);
                    $("#normalRate").text(normalCount + '人' + '(' + normalRate * 100 + '%' + ')');
                    $("#midRate").text(midCount + '人' + '(' + midRate * 100 + '%' + ')');
                    $("#heightRate").text(heightCount + '人' + '(' + heightRate * 100 + '%' + '）');
                    $("#warnRate").text(((parseFloat(midRate) + parseFloat(heightRate)) * 100).toFixed(2) + '%');
                    var tempDataArray = [[parseFloat(normalRate)], [parseFloat(midRate)], [parseFloat(heightRate)]];
                    for (var i = 0; i < tempDataArray.length; i++) {
                        realDataArray.push({ "data": tempDataArray[i], "color": "RGB(166,166,166)" });
                    }
        ...
        function LoadReportBasicInfo() {
            var grID = GetQueryString('grID');
            $.ajax({
                url: "Ajax/AutisticDisorder.ashx?OperationType=LoadReportBasicInfo",
                type: "GET",
                dataType: "text",
                data: { grID: grID },
                async: false,
