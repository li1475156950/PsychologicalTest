        ...
        public string CalculateParenthesesExpression(string Expression)
        {
            try
            {
                ...
                while (Expression.Length > 0)
                {
                    operand3 = "";
                    //取数字处理
                    if (Char.IsNumber(Expression[0]) || Expression[0].ToString().Equals("."))
                    {


                        while (Char.IsNumber(Expression[0]) || Expression[0].ToString().Equals("."))
                        {
                            //bool T = Char.IsNumber(Expression[0]);
                            //bool T2 = Expression[0].Equals(".");
                            //bool aa = T;
                            //bool bb = T2;
